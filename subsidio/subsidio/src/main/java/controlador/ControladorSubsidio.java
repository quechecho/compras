...
public class ControladorSubsidio implements ActionListener {
    ...
    try {
        String apellidoBuscado = ent.getTextapeEst().getText();
        vecBus.clear();
        
        // Buscar todos los estudiantes con el apellido especificado
        for (estudiantes est : admArr.getvecEst()) {
            if (est.getApeEst().toLowerCase().contains(apellidoBuscado.toLowerCase())) {
                vecBus.add(est);
            }
        }
        ...
        if (!vecBus.isEmpty()) {
            ...
            estudiantes primerResultado = vecBus.get(0);
            deshabilitarTxt();
            ent.getTextideEst().setText(primerResultado.getIdeEst());
            ent.getTextnomEst().setText(primerResultado.getNomEst());
            ent.getTextproSem().setText(String.valueOf(primerResultado.getProSem()));
            ent.getTextvalMatSem().setText(f3.format(primerResultado.getValMatSem()));
            habilitarCrud();
            
            if (vecBus.size() > 1) {
                JOptionPane.showMessageDialog(null, "Se encontraron " + vecBus.size() + " estudiantes con ese apellido. Se muestra el primero.",
                        "Múltiples resultados", JOptionPane.INFORMATION_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">No se encuentra ningún estudiante con ese apellido</p></html>",
                    "Atención", JOptionPane.WARNING_MESSAGE);
            ent.getTextapeEst().setText("");
            ent.getTextapeEst().requestFocus();
        }
    } catch (Exception ex) {
        JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">Error: " + ex.getMessage() + "</p></html>", 
                "Error", JOptionPane.ERROR_MESSAGE);
...
public void btnBuscarnomEst() {
    try {
        String nombreBuscado = ent.getTextnomEst().getText();
        vecBus.clear();
        
        // Buscar todos los estudiantes con el nombre especificado
        for (estudiantes est : admArr.getvecEst()) {
            if (est.getNomEst().toLowerCase().contains(nombreBuscado.toLowerCase())) {
                vecBus.add(est);
            }
        }
        ...
        if (!vecBus.isEmpty()) {
            ...
            deshabilitarTxt();
            ent.getTextideEst().setText(primerResultado.getIdeEst());
            ent.getTextapeEst().setText(primerResultado.getApeEst());
            ent.getTextproSem().setText(String.valueOf(primerResultado.getProSem()));
            ent.getTextvalMatSem().setText(f3.format(primerResultado.getValMatSem()));
            habilitarCrud();
            
            if (vecBus.size() > 1) {
                JOptionPane.showMessageDialog(null, "Se encontraron " + vecBus.size() + " estudiantes con ese nombre. Se muestra el primero.",
                        "Múltiples resultados", JOptionPane.INFORMATION_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">No se encuentra ningún estudiante con ese nombre</p></html>",
                    "Atención", JOptionPane.WARNING_MESSAGE);
            ent.getTextnomEst().setText("");
            ent.getTextnomEst().requestFocus();
        }
    } catch (Exception ex) {
        JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">Error: " + ex.getMessage() + "</p></html>", 
                "Error", JOptionPane.ERROR_MESSAGE);
...
public void btnBuscarproSem() {
    try {
        float promedioSemestre = Float.parseFloat(ent.getTextproSem().getText());
        vecBus.clear();
        
        // Buscar todos los estudiantes con el promedio especificado
        for (estudiantes est : admArr.getvecEst()) {
            if (est.getProSem() == promedioSemestre) {
                vecBus.add(est);
            }
        }
        ...
        if (!vecBus.isEmpty()) {
            ...
            if (vecBus.size() > 1) {
                JOptionPane.showMessageDialog(null, "Se encontraron " + vecBus.size() + " estudiantes con ese promedio. Se muestra el primero.",
                        "Múltiples resultados", JOptionPane.INFORMATION_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">No se encuentra ningún estudiante con ese promedio</p></html>",
                    "Atención", JOptionPane.WARNING_MESSAGE);
            ent.getTextproSem().setText("");
            ent.getTextproSem().requestFocus();
        }
    } catch (NumberFormatException ex) {
        JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">Error: Formato de número inválido</p></html>", 
                "Error", JOptionPane.ERROR_MESSAGE);
        ent.getTextproSem().setText("");
        ent.getTextproSem().requestFocus();
    } catch (Exception ex) {
...
public void btnBuscarvalmatSem() {
    try {
        float valorMatricula = Float.parseFloat(ent.getTextvalMatSem().getText());
        vecBus.clear();
        
        // Buscar todos los estudiantes con el valor de matrícula especificado
        for (estudiantes est : admArr.getvecEst()) {
        ...
        if (!vecBus.isEmpty()) {
            ...
            if (vecBus.size() > 1) {
                JOptionPane.showMessageDialog(null, "Se encontraron " + vecBus.size() + " estudiantes con ese valor de matrícula. Se muestra el primero.",
                        "Múltiples resultados", JOptionPane.INFORMATION_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">No se encuentra ningún estudiante con ese valor de matrícula</p></html>",
                    "Atención", JOptionPane.WARNING_MESSAGE);
            ent.getTextvalMatSem().setText("");
            ent.getTextvalMatSem().requestFocus();
        }
    } catch (NumberFormatException ex) {
        JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">Error: Formato de número inválido</p></html>", 
                "Error", JOptionPane.ERROR_MESSAGE);
...
// Métodos de ordenamiento adaptados al proyecto de subsidios
...
// Método de editar adaptado
public void btnEditar(){
    habilitarTxt();
    ent.getTextideEst().setEditable(false); // La identificación no se puede editar
    deshabilitarCrud(); // Deshabilitar otros botones CRUD mientras se edita
    visSub.getBtnValidar().setEnabled(false); // Deshabilitar validar durante edición
}

// Método de guardar adaptado
public void btnGuardar(){
    try {
        // Validar los datos antes de guardar
        if (val.validarNombre(ent.getTextnomEst().getText()) &&
            val.validarApellido(ent.getTextapeEst().getText()) &&
            val.validarNumeroRealPositivo(ent.getTextproSem().getText()) &&
            val.validarNumeroRealPositivo(ent.getTextvalMatSem().getText())) {
            
            // Actualizar los datos del estudiante en el vector
            ...
            JOptionPane.showMessageDialog(null, "<html><p style =\"color: green\">Registro actualizado correctamente</p></html>",
            ...
            JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">Error en la validación de los datos</p></html>",
        ...
        JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">Error al guardar: " + ex.getMessage() + "</p></html>",
                        "Error", JOptionPane.ERROR_MESSAGE);
    }
}

// Método de eliminar adaptado
public void btnEliminar(){
    try {
        int respuesta = JOptionPane.showConfirmDialog(null, 
            "¿Está seguro de que desea eliminar el registro del estudiante " + 
            admArr.getvecEst().get(i).getNomEst() + " " + 
            admArr.getvecEst().get(i).getApeEst() + "?",
        ...
        if (respuesta == JOptionPane.YES_OPTION) {
            admArr.eliminar(i);
            JOptionPane.showMessageDialog(null, "<html><p style =\"color: green\">Registro eliminado correctamente</p></html>",
                            "Éxito", JOptionPane.INFORMATION_MESSAGE);
            btnLimpiar();
            habilitarTxt();
            deshabilitarCrud();
            ...
            // Actualizar la vista si hay estudiantes
            if (!admArr.getvecEst().isEmpty()) {
                btnVer();
            } else {
        ...
        JOptionPane.showMessageDialog(null, "<html><p style =\"color: red\">Error al eliminar: " + ex.getMessage() + "</p></html>",
                        "Error", JOptionPane.ERROR_MESSAGE);
    }
}
