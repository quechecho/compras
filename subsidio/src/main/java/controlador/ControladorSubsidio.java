...
public class ControladorSubsidio {
    ...
    // Para mostrar elementos obtenidos de la lista
                ...
                if (val.validarApellido(ent.getjTextapeEst().getText())) {
                    ...
                    if (val.validarNumeroRealPositivo(ent.getjTextproSem().getText())) {
                        ...
                        if(val.validarNumeroRealPositivo(ent.getjTextvalMatSem().getText())) {
                            ...
                            // Crear el objeto estudiante con los datos validados
                            est = new estudiantes(ide, nom, ape, proSem, valMatSem);
                            
                            // Habilitar botones de lista enlazada
                            habilitarBtnLista();
    ...
    private void btnLimpiar() {
        ...
        // Limpiar también las variables auxiliares
        nom = "";
        ape = "";
        ide = "";
        proSem = 0;
        ...
        // Deshabilitar botones de lista
        deshabilitarBtnLista();
    }
    ...
    public void btnAddDespues() {
        if (DatoNodo()) {
            if (est != null) {
                ...
                estudiantes nuevoEst = new estudiantes(ide, nom, ape, proSem, valMatSem);
                if (admLista.InsertarDespues(nro, nuevoEst)) {
                    insercion();
                    btnLimpiar();
                } else {
    ...
    public void habilitarBtnLista(){
        ...
        visSub.getBtnDelInicio().setEnabled(true);
        visSub.getBtnDelFinal().setEnabled(true);
        visSub.getBtnDelDespues().setEnabled(true);
        visSub.getBtnObtener().setEnabled(true);
        visSub.getBtnContar().setEnabled(true);
        visSub.getBtnTodos().setEnabled(true);
    }
    ...
    public void deshabilitarBtnLista(){
        ...
        visSub.getBtnDelInicio().setEnabled(false);
        visSub.getBtnDelFinal().setEnabled(false);
        visSub.getBtnDelDespues().setEnabled(false);
        visSub.getBtnObtener().setEnabled(false);
        visSub.getBtnContar().setEnabled(false);
        visSub.getBtnTodos().setEnabled(false);
    }

    public void actionPerformed(ActionEvent e) {
        // Método requerido por la interfaz ActionListener si se implementa
    }
}